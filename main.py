...
def data_analysis_module():
    ...
    if st.button("🚀 开始分析", type="primary", disabled="df" not in st.session_state):
        ...
        with st.spinner("🔍 AI正在分析数据，请稍候..."):
            ...
            # 支持多种图表类型
            chart_types = ["bar", "line", "pie", "scatter", "box", "hist", "area"]
            for chart_type in chart_types:
                if chart_type in result:
                    create_chart(result[chart_type], chart_type)
